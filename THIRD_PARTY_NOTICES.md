...
自己完結形式の実行ファイルを配布する場合は、.NETランタイムとネイティブSQLiteなどのバイナリも含まれます。この一覧だけで配布時の表示を代替せず、実際の出力物に含まれるLICENSE・NOTICE・THIRD-PARTY-NOTICES等を保持し、配布内容に応じて確認してください。この公開準備ではソースのみを梱包します。
